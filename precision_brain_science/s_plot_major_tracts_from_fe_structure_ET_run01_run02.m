...
for isbj = 1:length(subject)
    ...
    for it = 1:length(trackingType)
        ...
        load(fasciclesClassificationSaveName)
        ...
        savedir  = fullfile(rootpath,'major_tracts');
        mkdir(savedir);
            
            fig_name = ['MAJOR_FAS_', sprintf('%s_96_%s_major_tracts_figs',subject{isbj},trackingType{it})];
            viewCoords = [-90,0];
            slice = [1 0 0];
            color = getColors;
